...
- `3_sudoku.py`: A Sudoku solver. It formulates a 9x9 Sudoku puzzle where constraints ensure each row, column, and 3x3 block contains unique numbers. Features a beautiful ASCII grid UI to highlight computed vs provided values.
- `4_cryptarithmetic.py`: A solver for the well-known cryptarithmetic puzzle `TWO + TWO = FOUR`, complete with a structured box-drawing CLI display.
...
- The other scripts will calculate the results and print their solutions strictly to the console (with `3_sudoku.py` and `4_cryptarithmetic.py` displaying visually clean, colored ASCII UX graphics!).
